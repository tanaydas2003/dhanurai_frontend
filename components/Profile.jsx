  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/profile', {
        headers: { Authorization: `Bearer ${token}` }, // Correct syntax with template literal
      });
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar
        position="fixed"
        sx={{
          width: { sm: `calc(100% - ${drawerWidth}px)` }, // Fixed calc usage
          ml: { sm: `${drawerWidth}px` }, // Fixed calc usage
        }}
      >
        <Toolbar>
          {isMobile && (
